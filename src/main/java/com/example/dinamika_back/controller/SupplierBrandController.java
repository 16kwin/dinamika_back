...
import com.example.dinamika_back.service.SupplierBrandColumnSettingsService;
import com.example.dinamika_back.service.SupplierBrandService;
...
@RestController
@RequestMapping("/api/supplier-brands")
...
public class SupplierBrandController {
    ...
    @GetMapping
    public ResponseEntity<SupplierBrandListResponse> getAll(@RequestParam(required = false) Integer userId) {
        return ResponseEntity.ok(supplierBrandService.getAllWithSettings(userId));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(@RequestBody CreateSupplierBrandRequest request) {
        return ResponseEntity.ok(supplierBrandService.create(request));
    }

    @PatchMapping("/{uid}")
    public ResponseEntity<Map<String, Object>> update(@PathVariable UUID uid,
            @RequestBody UpdateSupplierBrandRequest request) {
        return ResponseEntity.ok(supplierBrandService.update(uid, request));
    }

    @DeleteMapping("/{uid}")
    public ResponseEntity<Void> delete(@PathVariable UUID uid) {
        supplierBrandService.delete(uid);
    ...
    @GetMapping("/events")
    public ResponseEntity<List<SupplierBrandEventLogDto>> getAllEvents() {
        return ResponseEntity.ok(supplierBrandService.getAllEvents());
    }

    @GetMapping("/{uid}/events")
    public ResponseEntity<List<SupplierBrandEventLogDto>> getEvents(@PathVariable UUID uid) {
        return ResponseEntity.ok(supplierBrandService.getEvents(uid));
    }
    ...
    public ResponseEntity<byte[]> exportExcel(@RequestBody Map<String, Object> request) throws Exception {
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"supplier-brands.xlsx\"")
                .body(excel);
    }
    ...
    public ResponseEntity<byte[]> exportWord(@RequestBody Map<String, Object> request) throws Exception {
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"supplier-brands.docx\"")
                .body(word);
    }
