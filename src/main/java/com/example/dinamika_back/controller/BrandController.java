...
import com.example.dinamika_back.service.BrandColumnSettingsService;
import com.example.dinamika_back.service.BrandService;
...
@RestController
@RequestMapping("/api/brands-crud")
...
public class BrandController {
    ...
    @GetMapping
    public ResponseEntity<BrandListResponse> getAll(@RequestParam(required = false) Integer userId) {
        return ResponseEntity.ok(brandService.getAllWithSettings(userId));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(@RequestBody CreateBrandRequest request) {
        return ResponseEntity.ok(brandService.create(request));
    }

    @PatchMapping("/{uid}")
    public ResponseEntity<Map<String, Object>> update(@PathVariable UUID uid, @RequestBody UpdateBrandRequest request) {
        return ResponseEntity.ok(brandService.update(uid, request));
    }

    @DeleteMapping("/{uid}")
    public ResponseEntity<Void> delete(@PathVariable UUID uid) {
        brandService.delete(uid);
    ...
    @GetMapping("/events")
    public ResponseEntity<List<BrandEventLogDto>> getAllEvents() {
        return ResponseEntity.ok(brandService.getAllEvents());
    }

    @GetMapping("/{uid}/events")
    public ResponseEntity<List<BrandEventLogDto>> getEvents(@PathVariable UUID uid) {
        return ResponseEntity.ok(brandService.getEvents(uid));
    }
    ...
    public ResponseEntity<byte[]> exportExcel(@RequestBody Map<String, Object> request) throws Exception {
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"brands.xlsx\"")
                .body(excel);
    }
    ...
    public ResponseEntity<byte[]> exportWord(@RequestBody Map<String, Object> request) throws Exception {
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"brands.docx\"")
                .body(word);
    }
