...
public class ManufacturerController {
    ...
    public ResponseEntity<ManufacturerMediaDTO> uploadImage(@PathVariable UUID uid,
            @RequestParam("file") MultipartFile file) {
        try {
            return ResponseEntity.ok(manufacturerService.uploadImage(uid, file));
        } catch (IOException e) {
    ...
    public ResponseEntity<ManufacturerDocumentDTO> uploadDocument(@PathVariable UUID uid,
        ...
        try {
            return ResponseEntity.ok(manufacturerService.uploadDocument(uid, documentName, file));
        } catch (IOException e) {
            return ResponseEntity.internalServerError().build();
        }
    }
    ...
    public ResponseEntity<byte[]> exportWord(@RequestBody Map<String, Object> request) throws Exception {
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"manufacturers.docx\"")
                .body(word);
    }
