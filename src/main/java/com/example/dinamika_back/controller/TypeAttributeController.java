...
import com.example.dinamika_back.service.TypeAttributeColumnSettingsService;
import com.example.dinamika_back.service.TypeAttributeService;
...
@RestController
@RequestMapping("/api/type-attributes-crud")
...
public class TypeAttributeController {
    ...
    @GetMapping
    public ResponseEntity<TypeAttributeListResponse> getAll(@RequestParam(required = false) Integer userId) {
        return ResponseEntity.ok(typeAttributeService.getAllWithSettings(userId));
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(@RequestBody CreateTypeAttributeRequest request) {
        return ResponseEntity.ok(typeAttributeService.create(request));
    }

    @PatchMapping("/{uid}")
    public ResponseEntity<Map<String, Object>> update(@PathVariable UUID uid, @RequestBody UpdateTypeAttributeRequest request) {
        return ResponseEntity.ok(typeAttributeService.update(uid, request));
    }

    @DeleteMapping("/{uid}")
    public ResponseEntity<Void> delete(@PathVariable UUID uid) {
        typeAttributeService.delete(uid);
    ...
    @GetMapping("/events")
    public ResponseEntity<List<TypeAttributeEventLogDto>> getAllEvents() {
        return ResponseEntity.ok(typeAttributeService.getAllEvents());
    }

    @GetMapping("/{uid}/events")
    public ResponseEntity<List<TypeAttributeEventLogDto>> getEvents(@PathVariable UUID uid) {
        return ResponseEntity.ok(typeAttributeService.getEvents(uid));
    }
