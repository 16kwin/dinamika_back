// StationTypeColumnSettingsService.java
package com.example.dinamika_back.service;

import com.example.dinamika_back.model.UserStationTypeColumnSettings;
import com.example.dinamika_back.repository.UserStationTypeColumnSettingsRepository;
...
public class StationTypeColumnSettingsService {

    private final UserStationTypeColumnSettingsRepository repository;

    public String getColumnsJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserStationTypeColumnSettings::getColumnsJson)
                .orElse(null);
    }

    public String getFiltersJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserStationTypeColumnSettings::getFiltersJson)
                .orElse("{}");
    }

    public String getSortJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserStationTypeColumnSettings::getSortJson)
                .orElse("{}");
    }

    @Transactional
    public void saveColumnsJson(Integer userId, String columnsJson) {
        UserStationTypeColumnSettings settings = getOrCreate(userId);
    ...
    private UserStationTypeColumnSettings getOrCreate(Integer userId) {
        return repository.findByUserId(userId)
                .orElseGet(() -> UserStationTypeColumnSettings.builder()
                        .userId(userId)
                        .columnsJson("{}")
                        .filtersJson("{}")
                        .sortJson("{}")
