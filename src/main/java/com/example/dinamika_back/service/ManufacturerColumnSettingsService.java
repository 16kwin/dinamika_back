...
import com.example.dinamika_back.repository.UserManufacturerColumnSettingsRepository;
...
public class ManufacturerColumnSettingsService {

    private final UserManufacturerColumnSettingsRepository repository;

    public String getColumnsJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserManufacturerColumnSettings::getColumnsJson)
                .orElse(null);
    }

    public String getFiltersJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserManufacturerColumnSettings::getFiltersJson)
                .orElse("{}");
    }

    public String getSortJson(Integer userId) {
        return repository.findByUserId(userId)
                .map(UserManufacturerColumnSettings::getSortJson)
                .orElse("{}");
    }

    @Transactional
    public void saveColumnsJson(Integer userId, String columnsJson) {
        UserManufacturerColumnSettings settings = repository.findByUserId(userId).orElse(null);
        if (settings == null) {
            settings = UserManufacturerColumnSettings.builder()
                    .userId(userId)
